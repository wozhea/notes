...
rx = [zeros(50,1); rx_signal; end_noise];
...
% 只是对前导加噪，没有定时等信号处理操作
% 160+32+64;
l_ltf_to_fft_index = 193;%%循环移位，exp(2pi*k/N),左偏为-
l_ltf_to_fft = rx_signal(l_ltf_to_fft_index:l_ltf_to_fft_index+63);
l_ltf_fft =fft(l_ltf_to_fft)/(64/sqrt(52));
l_ltf_fft_indexmap = [l_ltf_fft(33:64,1);l_ltf_fft(1:32)];
l_ltf_fft_data = [l_ltf_fft_indexmap(7:32);l_ltf_fft_indexmap(34:59)];
H = l_ltf_fft_data./LongTrain;
% a= angle(l_ltf_fft_data);
% a = unwrap(angle(l_ltf_fft_data),pi);
% b = unwrap(angle(LongTrain),pi);
% a = angle(l_ltf_fft_data);
% b = angle(LongTrain);

% figure(1),plot(angle(H));
% figure(2),plot(abs(H));
        ...
        % demodulate
        DemodSeq = de2bi(qamdemod(Data_seq*NormFactor,2^modulation_level),modulation_level);
        SerialBits = reshape(DemodSeq,size(DemodSeq,1)*modulation_level,1).';
        
        if InterleaveBits
           deint_bits = rx_deinterleave(SerialBits, 48, modulation_level);
        else
