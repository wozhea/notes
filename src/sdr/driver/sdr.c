...
static int openwifi_dev_probe(struct platform_device *pdev)//内核注册时，WIFI配置存储量的初始化和注册
{
	struct ieee80211_hw *dev;//wifi硬件抽象结构
	struct openwifi_priv *priv;//openwifi软件数据结构
	int err=1, rand_val;
	const char *chip_name, *fpga_model;//存储fpga型号和信息
	u32 reg, i;//, reg1;

	struct device_node *np = pdev->dev.of_node;//设备树节点

	struct device *tmp_dev;//临时设备指针
	struct platform_device *tmp_pdev;//临时平台设备指针
	struct iio_dev *tmp_indio_dev;//临时IIO设备指针
	...
	if (np) {
		...
		match = of_match_node(openwifi_dev_of_ids, np);//检查设备树节点 np 是否与驱动的设备 ID 匹配
		if (match) {
	...
	dev = ieee80211_alloc_hw(sizeof(*priv), &openwifi_ops);//为wifi硬件分配内存，并将大小为sizeof(*priv)的私有数据（priv）绑定到硬件结构中
	if (!dev) {
	...
	err = of_property_read_string(of_find_node_by_path("/"), "model", &fpga_model);//从设备树中读取根节点（/）的 model 属性，并将其存储到 fpga_model 中
	if(err < 0) {
...
//看起来现在的设备树只有7z020，默认判断为SMALL_FPGA了。。。
	...
	tmp_dev = bus_find_device( &spi_bus_type, NULL, "ad9361-phy", custom_match_spi_dev );//使用bus_find_device在 spi_bus_type 总线上查找名为 "ad9361-phy" 的设备。
	if (tmp_dev == NULL) {
	...
	priv->ad9361_phy = ad9361_spi_to_phy((struct spi_device*)tmp_dev);//将找到的设备转换为AD9361 PHY驱动的物理指针，用于后续操作。
	if (!(priv->ad9361_phy)) {
	...
	tmp_dev = bus_find_device( &platform_bus_type, NULL, "cf-ad9361-dds-core-lpc", custom_match_platform_dev );//使用 bus_find_device 查找平台总线上的 "cf-ad9361-dds-core-lpc" 设备
	if (!tmp_dev) {
	...
	tmp_pdev = to_platform_device(tmp_dev);//将找到的设备(axi_ad9361设备)转换为 platform_device
	if (!tmp_pdev) {
	...
	tmp_indio_dev = platform_get_drvdata(tmp_pdev);//使用platform_get_drvdata获取设备(axi_ad9361)的私有数据
	if (!tmp_indio_dev) {
	...
	priv->dds_st = iio_priv(tmp_indio_dev);//使用iio_priv获取IIO设备的私有数据，并存储到priv->dds_st中
	if (!(priv->dds_st)) {
