...
### zynq启动流程
ZYNQ组合了一个双核 ARM Cortex-A9 处理器 和 一个现场可编程门阵列
![start_process.JPG](/screenshot/start_process.JPG);
BootRom，无法更改，先完成对SD、NAND、NOR、QSPI Flash 等的初始化，使得ARM核能够成功访问并使用这些外设，读取不同MIO启动管脚电平，决定从SD卡、FLASH、JTAG等不同存储介质读取所需镜像，加载到ARM核的OCM（片上存储器）。
![fsbl.JPG](screenshot/fsbl.JPG)
fsbl (first stage boot loader),第一阶段加载，引导操作系统或后续运行裸机程序，直接从hdf/xsa文件的硬件描述中导出.第1步对PS端初始化，包括DDR，MIO等初始化。第2步PL下载bit流。第3步是加载SSBL或裸跑程序到DDR内存中，第4步ARM会跳转到DDR中执行所加载的程序。
zynq的u-boot和SSBL功能相当
fsbl、bit流、u-boot.elf文件可制作BOOT.BIN文件
...
变量定义:[存储类型] 数据类型 标识符 = 值
        ...
        存储类型：auto static register extern(说明型)
                 auto：默认，块内，自动分配空间，自动回收空间
                 register：(建议型，由编译器如gcc决定是否编译为register类型)寄存器类型，只能定义局部变量，不能定义全局变量；块内，大小有限制，只能定义32位大小的数据类型，如double就不可以，寄存器没有地址，所以一个寄存器类型的变量无法打印出地址查看或使用。
...
# Linux 硬件 数电 信号处理 杂七杂八
### 场效应管
N沟道CMOS管主要原理：在P型衬底上有两个N型沟道，分别为源S极和漏D极，当栅源电压V_GD大于导通电压时产生电场，当V_SD大于零时产生电流
N沟道MOS管和P沟道MOS导通条件区别在于栅漏电压正负，栅漏电压符合导通电压时，源漏电压大于零时产生MOS管导通，产生电流
...
SPI没有读和写之分，在每个 Clock 周期内，SPI 设备都会发送并接收一个 bit 大小的数据(不管主设备好还是从设备)，即每个时钟下SPI设备都是在交换数据

CPOL（Clock Polarity）时钟极性，表示空闲时时钟是高还是低电平，0表示低电平。CHPA(Clock Phase)时钟相位决定第一个边沿还是第二个边沿开始进行采样，0表示第一个边沿采样。
