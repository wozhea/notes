# 面试经验
## 自我介绍
面试官好，首先非常感谢能给予我这次面试机会，我是刘哲汗，25岁，来自浙江舟山，本科和硕士在南京理工大学就读通信工程，通信与信息系统专业，主要研究方向是信号处理和无线通信、无线感知。

硕士期间，我参与了多个无线通信系统的相关项目。一个是为某概念的无人机群开发的图传和遥控的通信载荷。在项目开发和联调的过程中发现了很多，该系统设计上存在的问题和可以提升的地方，针对这些问题开展了另一个基于wifi的通信系统的开发，并完成了无线感知相关的课题。这些经历让我感受到了无线通信从书本理论，到具体实现的差别，也感受到了在团队协作时，责任感和沟通的重要性，还有关于自身从技术上，和其他各方面的不足。

我今天应聘的岗位是无线通信嵌入式开发和FPGA开发，当看到无人机飞在天空中，通过自己参与开发的数据链传下来清晰的视频时，心中是有一些成就感的和感动的，所以对该岗位有一些兴趣。而且我有较为完整通信系统开发经验，和相关的无线通信知识储备，希望将我的设计和实战经验应用到更具挑战性的工程中，也期待能在具体的工作中得到锻炼，谢谢。




## 感想记录
FPGA讲究的是广度，便捷性，随改随用。IC设计讲究的是深度，要手搓代码，IP核调没用，要写一点能被问的东西。装一下IC设计的VCS和Verdi软件，配一下环境。
...
有什么想问的？
1) 工资：①工资的构成（几薪，超出12薪的部分会写在劳动合同里吗） ②基资（底薪）是多少（月薪是多少，年薪是多少，税前、税后、还是总包）③加费怎么算 ④绩效怎么考核（月/季/年） ，绩效考核的标准，绩效考核的结果是什么（影响薪资，年终奖还是其它）⑤出差补贴怎么算（出差是实报实销还有补贴，出差情况怎么样，频不频繁）⑥工资发放时间：每月几号发 ⑦涨薪：每年几次调薪，涨幅怎么样
2) 福利：食住方面：吃住是公司提供还是说有补贴（餐补，房补一类），人才补贴，节日补贴，有无食堂/健身房/班车/宿舍/免费体检
3) 五险一金：多少比例，多少基数（公积金比例），有无补充商业保险
4) 职业发展：入职职级，入职培养计划：新人培养模式是什么，试用期多久，试用期薪资，实习是否抵扣试用期，转正考核标准、方式、转正率，试用五险一金正常缴纳吗；晋升情况怎么样，途径是什么；近几届应届生流动情况
5) 假期：单双休情况，年假情况（应届生入职会有吗），有无带薪病假，
6) 工作时间：上下班时间，午休时间，打卡：迟到，漏打卡（处理方式：会直接扣钱吗）
7) 劳动合同和谁签，直接与公司还是外包；是否可以办理落户
8) 工作地点，同事工龄，部门人员流动率
## 项目中遇到的问题
双ISERDES+双IODELAY2逐位去偏斜

无人机多路传输
本来是要求3路视频传输，在做完资源评估后，选用了ZCU27DR主板作为核心板，资源是够的，后面需求变更，又来了个飞机厂家和光电载荷厂家和原本的一起做实验，导致需求变成五路同时图传。在例化后发现FPGA的资源不够用了。
然后开发板本来是一个雷达抗干扰项目使用的板子，板子上总共有三片FPGA，两块ZCU27DR和一块中控Kintex7。此时有好几种可以解决问题的思路，第一种是两块带AD/DA的ZCU27DR分别负责3路和2路的下行视频处理，但是系统上行通信采用的是简单的信道，只有一个频点，多台设备都能接收，用IP地址区分是否是给自己的信息，如果这样需要两个低噪放。第二种是优化资源，在某些地方牺牲精度来换取一些资源。第三种，是最后我们采用的，把一路采样数据经信道化接收机和上下变频模块后的输出传到另一块FPGA，再把解调成功后的输出传回来。具体要传过去的数据有，12位的I路数据，12位的Q路数据，时钟，复位，每秒的接收机复位信号，总共27根，传回来8位的bit数据和1位有效信号2bit同步标志位(好像不需要传输)，共11位。实际做的时候发现，有时候这一路会解调不出来，10次启动可能会有一两次完全解调出来的包校验一直不通过，通过加探针的方式发现传输到第二片FPGA的时候，ADC数据在某些位上出现了延迟一个时钟。
然后要复现这个问题，发现在接收失败时，传输的数据在数据高位发生了错位，而且是一直错位，发生错位的位置也一直是在几根线上，在低位发生错位的时候并不影响解调。初步判断是PCB走线的长度不一致、管脚延迟不一致，导致的传输时延，第二片FPGA的时钟存在相差，如果时钟上升沿正好卡在产生偏差的位置，就产生了错位。
从系统的角度讲可以通过重启，保证接收站到正常工作模式，但从解决问题的角度讲，要降低出现错误的概率。于是通过对其他高位线添加了set_input_delay的时序约束后，出现问题的概率降低了。
...
变量定义:[存储类型] 数据类型 标识符 = 值
        ...
        存储类型：auto static register extern(说明型)
                 ...
                 static:静态型，局部有效，自动初始化为0值或空值，并且其变量的值有继承性,只被定义一次。常用于修饰变量或函数，修饰函数时防止对外扩展，只有当前文件可以调用。
                修饰三种数据：
                1、函数内部的变量
                2、函数外部的变量
                3、函数的修饰符
...
### 内存分布
    ...
    内核空间    应用程序不允许访问
---------
    栈空间      局部变量,函数内部使用的变量，函数一旦返回就释放   read_write
---------
    运行时的堆空间  运行时可以自由分配malloc(大小)和释放free(p)的空间  malloc(),输入参数分配指定大小B，返回分配好的地址
----------
    全局数据空间 （初始化DATA和未初始化BSS）
    只读数据段  “hello world”            Read_only
    代码段                               Read_only
----------
...
值传递，上层调用者可以保护自己空间的值不被修改的能力

地址传递，调用者可以让下层子函数修改自己空间值
...
#### APB
APB传输都由主机发起，从机相对而言只负责配合。


#### AXI 
AXI协议的五个通道，写数据地址AW，写数据W，写响应B，读数据地址AR，读数据R

#### AXIS FIFO
AXIS五根线，tdata,tkkep,tlast,tvalid,tready(in)

在例化的FIFO核中,32位数据，4位keep，
...
#### 机载软件
初始化组包参数如IP地址和端口号，远程控制功能。
通过pthread_create创建多个线程，分别处理不同的数据通路和功能。
1.net_recv：网口接收线程，死循环从以太网口接收UDP包，过滤掉与本网口MAC相关的包后，入队列。
2.ps2pltransfer，从队列取数据，将BUFF写入PL端FIFO
3.net_trans，从rx_fifo读取FPGA解析出的数据，判断是否为UDP包，根据目的IP和端口号将控制指令写入RS232\RS422文件句柄，特定指令做射频开关，其他数据通过以太网口发出。
4.rs422_master_recv/rs232_master_recv：串口接收线程，死循环将串口收到的数据封装成UDP包后入队列，
...
Zynq UltraScale+ RFSoC data converter地面ADC采用4GSPS采样速率，一个ADC以AXIS形式两路IQ数据250M的128位数据进行输出,IP核内已完成2倍抽取至2G（8*16*250M），此时每一位I或Q路数据为16位，在adc_interface模块中截取12位数据，输出8路24位IQ数据，[ Q[11:0],I[11:0]]，给到16通道信道化接收机channelizer_receiver_complex_16  
...
信道化输出后已变成16路信道的输出，一个信道带宽125M，单边62.5M，选用了两个信道分别进行上下变频和滤波处理，输出该路信号的单载波数字调制信号。
例化五路接收
...
同步头  遥测  帧长  帧计数  有效数据帧头  有效数据帧长  CRC  LDPC校验位；
一帧共1024B,同步头共4B，用于Farrow插值+位同步，不进行编码；遥测信息头共13B，2B帧长，2B帧计数；数据位共875B，其中2B帧头EB90，2B数据长度，尾部32位CRC，有效payload共839B，数据不足则填充33；采用CCSDS(8176,7156，7/8)LDPC编码，892B字节加18bit的0共7154位数据作为编码器输入，编码后不发送首18bit，尾补2bit的0共输出8160bit（8176-18+2） 1020B数据输出。

dpram用于存储待发送的数据，统计待发送的数据个数。
...
符号速率6Msps,先进行4级内插到24Msps，再进行2倍FIR内插滤波到48M。8倍过采样，采样速率48M。
...
### QPSK调制解调
PS和PL交互的FIFO,PS端fifo32位512深度，PL端32位16K深度，用于缓存网络包数据。

#### PSPL接口
发射机，32位bit数据通过axis_stream格式向外输出。通过arm_intf将数据接入调制解调主模块的fifo。主模块输出待发送8位数据深度，接口模块输出数据和valid信号。
主要时序：复位后进入空闲状态，当主模块待发送数据小于8K时进入读取axis状态，锁存一个32位的数据后，分4次写入主模块，再次进入空闲状态。

接收机，数据先进入recv_qpsk，按照帧格式进行遥测数据和有效数据接收，并进行crc校验。
之后送入mac_anlyze，按照网络包长，EB90和物理包长长提取有效载荷，将分开的网络包拼成一个
之后送入组axis数据状态机。
