...
CRC字段只校验PSDU中的DATA，CRC后跟TAIL和PAD。（有必要自己再加一次CRC吗？？？）
一个普通OFDM符号处理周期3.78us,小于4us符号周期

进入到数据位后，64位的数据首先经过计数一个一个读入变成串行数据，通过组合逻辑判断当前state1和state11状态选择加扰操作，变成可以统一OFDM符号信道编码的比特。先送入卷积器变为串行2位比特，在这之前都是可以顺序执行的，随后要送入打孔和交织器，需要处理时间，所以在卷积和打孔之间需要FIFO，通过FIFO状态位控制前后模块使能。打孔和交织通过输入速率和比特位置，输出查找表行列地址和是否打孔信息，直接略过打孔位，通过地址读取交织后的待调制的比特。待调制的比特通过查找表完成量化和数字映射调制。通过计数和组合逻辑生成当前符号的导频符号，将数据和导频和0插入到对应位置。将数据送入IFFT模块，输出最后的OFDM符号。
...
power_trigger，接收机检测能量大于能量门限
三种情况复位
receiver_rst_internal，监测直流偏移，对输入数据符号位进行32点平均，计算直流分量，若为0则交替选择-1和1避免异常，若直流分量大于阈值，复位。
equalizer_monitor_rst，均衡器归一化星座点的绝对值过小，无法解调。
signal_len_rst，解调出来的物理头signal大于或小于协议规定长度。
